...
st.set_page_config(page_title="Today's Workout", page_icon="📅")
...
if active_session:
    ...
    if not plan:
        ...
        # Note: If there is an active session (snapshot), it is caught above by `get_active_session`.
        # If we are here, it means either NO active session, or the active session row was not found (rare if logic aligns).
        # However, `get_day_plan` returns the row that `get_active_session` might also return if it's active.
        # But `get_active_session` filters by `status='ACTIVE'`.
        # `get_day_plan` returns whatever is there for the date.
        
        if plan.get('status') == 'COMPLETED':
            st.success(f"✅ Workout Completed: {plan['name']}")
            st.caption("Great job! See you tomorrow.")
            # We can show details (read-only) or just hide everything.
            # Requirement: "Today page no longer shows active runner." -> We are in "Planner Mode" here so runner is hidden.
            # But we should also hide "Start Workout".
            ...
            # PLANNED or otherwise not Active/Completed (e.g. if we cleared active but didn't set completed?)
            template_id = plan['template_id']
            template = get_template(template_id)
